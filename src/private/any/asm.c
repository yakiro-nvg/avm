...
extern const achunk_t CHUNK_HEADER;
...
AINLINE aasm_constant_t* constants_of(aasm_prototype_t* p)
{
    ...
    const aimport_t* imports)
{
    ...
    sz = sizeof(uint32_t) + 1 +
        strlen(any_st_to_string(st, p->source));
    sz += sizeof(uint32_t) + 1 +
    ...
    const aasm_t* self, int32_t parent, int32_t* psz)
{
    ...
    *psz += sizeof(aprototype_t) +
        ...
        p->num_nesteds * sizeof(aprototype_t*);
...
AINLINE avalue_t to_value(aasm_constant_t c, aasm_t* self, aprototype_t* header)
{
    avalue_t v;
    v.tag.tag = ATB_NIL;
    switch (c.b.type) {
    ...
    case ACT_REAL:
        ...
        break;
    case ACT_STRING:
        v.tag.tag = ATB_CONST_STRING;
        v.v.cs = chunk_add_str(self, header, c.s.ref);
        break;
    }
    return v;
}
    ...
    int32_t strings_sz)
{
    ...
    // add constants
    for (i = 0; i < p->num_constants; ++i) {
        cp.constants[i] = to_value(c->constants[i], self, header);
    }
...
AINLINE aasm_constant_t from_value(avalue_t v, aasm_t* self, const aprototype_t* p)
{
    aasm_constant_t c;
    ...
    } else if (v.tag.tag == ATB_CONST_STRING) {
        c.b.type = ACT_STRING;
        c.s.ref = any_asm_string_to_ref(self, cp_string(p, v.v.cs));
        return c;
    }
    ...
    aasm_t* self, const achunk_t* input, int32_t* offset)
{
    ...
    ap->source = any_asm_string_to_ref(self, cp_string(p, p->source));
    ...
    for (i = 0; i < p->num_constants; ++i) {
        any_asm_add_constant(self, from_value(cp.constants[i], self, p));
    }

    for (i = 0; i < p->num_imports; ++i) {
        aimport_t imp = cp.imports[i];
        any_asm_add_import(
            self, cp_string(p, imp.module), cp_string(p, imp.name));
    }
...
int32_t any_asm_load(aasm_t* self, const achunk_t* input)
{
    ...
    if (memcmp(&CHUNK_HEADER, input, sizeof(achunk_t)) == 0) {
        int32_t offset = sizeof(achunk_t);
...
void any_asm_save(aasm_t* self)
{
    ...
    memcpy(self->chunk, &CHUNK_HEADER, sizeof(achunk_t));
    self->chunk_size += sizeof(achunk_t);
...
int32_t any_asm_add_constant(aasm_t* self, aasm_constant_t constant)
{
    aasm_prototype_t* p = any_asm_prototype(self);
    aasm_reserve_t sz;
...
int32_t any_asm_add_import(aasm_t* self, const char* module, const char* name)
{
    aimport_t import;
    aasm_prototype_t* p;
    aasm_reserve_t sz;

    import.module = any_asm_string_to_ref(self, module);
    import.name = any_asm_string_to_ref(self, name);
    p = any_asm_prototype(self);
...
int32_t any_asm_module_push(aasm_t* self, const char* name)
{
    aasm_prototype_t* p = NULL;
    int32_t idx;
    astring_ref_t symbol = any_asm_string_to_ref(self, name);
