...
ANY_API void astd_lib_add_string(aloader_t* l);
...
static AINLINE aint_t any_string_length(aactor_t* a, aint_t idx)
{
    avalue_t* v = a->stack.v + idx;
    agc_string_t* s = AGC_CAST(agc_string_t, &a->gc, v->v.heap_idx);
    return s->hal.length;
...
static AINLINE aint_t any_string_hash(aactor_t* a, aint_t idx)
{
    avalue_t* v = a->stack.v + idx;
    agc_string_t* s = AGC_CAST(agc_string_t, &a->gc, v->v.heap_idx);
