...
\brief
The `apid_t` is 32-bits wide, consists of two parts are `index` and `generation`,
...
*/
ANY_API int32_t any_vm_startup(
    avm_t* self, int32_t idx_bits, int32_t gen_bits, aprocess_t* procs);

/// Graceful cleanup the VM.
ANY_API void any_vm_shutdown(avm_t* self);

/** Lock for alive process.
\return NULL if that is not found or died.
*/
AINLINE aprocess_t* any_vm_proc_lock(avm_t* self, apid_t pid)
{
    int32_t idx = apid_idx(self->_idx_bits, self->_gen_bits, pid);
...
\return NULL if that is not found or died.
*/
AINLINE aprocess_t* any_vm_proc_lock_idx(
    avm_t* self, int32_t idx, ascheduler_t* owner)
{
    aprocess_t* p = self->_procs + idx;
...
/// Unlock a process.
AINLINE void any_vm_proc_unlock(aprocess_t* p)
{
#ifdef ANY_SMP
    amutex_unlock(&p->mutex);
#else
...
*/
ANY_API aprocess_t* any_vm_proc_allocate(avm_t* self);

/// Return this process to the pool.
AINLINE void any_vm_proc_free(aprocess_t* p)
{
    p->flags |= APF_DEAD;
}
...
is ok to run this function on background thread, if you are facing timing problems.
...
*/
ANY_API void any_vm_migrate_messages(avm_t* self, ascheduler_t** schedulers);
