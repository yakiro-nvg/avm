...
def to_cdef(filename, typedefs):
    """Generate C declaration from source code.
    Args: typedefs (str[]): expected typedefs.
    """
    typedefs = [
    ...
    ast = parse_file(
        ...
        cpp_args=[
            '-E', '-DATEST',
            '-I' + os.path.join(PATH, '../externals/pycparser/' +
...
def load_ffi(filename, typedefs):
    """Generate CFFI from source code.
    Args: typedefs (str[]): expected typedefs.
    """
    ...
    with open(filename, 'r') as f:
        ...
        if coverage:
            ...
            src += '\nvoid __gcov_flush(void); \
                void flush_coverage(void) { __gcov_flush(); }'
        else:
